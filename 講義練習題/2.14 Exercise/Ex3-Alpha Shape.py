...
def printl(x):    # 輸出單行不換行
    print(x, end = "")

def up(y):    # 把數字轉換為大寫英文
    return (chr(y + 64))
    
def low(y):    # 把數字轉換為小寫英文
    return (chr(y + 96))

def uplist(ly):    # 把數字鎮列轉換為大寫英文
    lr = list(up(int(i)) for i in ly)
    return (lr)

def lowlist(ly):    # 把數字陣列轉換為小寫英文
    lr = list(low(int(i)) for i in ly)
    return (lr)

def ULlist(ly):    # 把數字陣列轉換為交替的大小寫英文 (先大寫在小寫)
    lr = list()
    i = 1
    while (i <= len(ly)):
        if (i % 2):
...
def reULlist(ly):    # 把數字陣列轉換為交替的大小寫英文 (先小寫在大寫)
    lr = list()
    i = 1
    while (i <= len(ly)):
        if (i % 2):
...
lx = list(str(i) for i in range(1,x+1))    # 建立長度為x的數字陣列
ln = list(str(i) for i in range(1,x+1,2))    # 建立長度為n的數字陣列
...
while (i <= (x+(x%2)-1)):
    if (((i*2)-1) <= x):    # 上半部
        if (i % 2):    # 奇數行
            ...
            printl(" " * (n-i) + f"{up(2*i-1)}" * ((i*2)-1) + " " * (n-i))    # 1號
            printl(" " * 2 + " " * (n-i) + f"{up(2*(n-i)+1)}" * ((i*2)-1) + " " * (n-i))    # 2號
            printl(" " * 2 + " " * (i-1) + f"{up((2*n-1)-(2*(i-1)))}" * (((n-i)*2)+1) + " " * (i-1))    # 3號
            printl(" " * 2 + " " * (i-1) + f"{up(2*i-1)}" * (((n-i)*2)+1) + " " * (i-1))    # 4號
